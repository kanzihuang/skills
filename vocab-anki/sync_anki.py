...
Usage:
    python sync_anki.py input.json --deck "The Little Prince Vocabulary"
    python sync_anki.py input.json --deck "English" --dry-run
    python sync_anki.py input.json --deck "English" --no-audio
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--deck", "-d", required=True, help="Target Anki deck name"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def build_note_entry(
    ...
    """
    safe = safe_filename(word_data["word"])
...
def sync(
    ...
    # 3. Get existing word map from Anki
    print(f"\nQuerying existing cards in deck...")
    existing = ac.get_word_map(deck_name)
    print(f"  Found {len(existing)} existing cards")

    # 4. Identify new words
    new_words = []
    skipped_words = []
    for w in words:
