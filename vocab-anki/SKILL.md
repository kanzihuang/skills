...
> Steps 2A（句子匹配）、2B（句子选择）、2C（IPA 预填充）、2D（生成释义+IPA）、2E（内容验证）、2F（DeepL 翻译）、2G（预下载音频）、2H（同步）与 vocab-book 共享。
...
> **必须是英文原版**，不能使用中英双语版。双语版中的中文翻译段落、西里尔字母作者名、guillemet（«»）书名号会严重污染句子匹配结果。`match_sentences.py` 在 Step 2A-c 遇到此类文本直接拒绝。

### 2A-b. 拉取源文本（优先 curl 直链，WebFetch 仅作兜底）：**

> **WebFetch 有严格引用字数限制（~125 字符/条），不适合拉取全书文本。必须优先用 `curl -sL` 直接下载。**

```bash
# 优先方案：curl 直链下载纯文本文件
curl -sL --max-time 60 '<2A-a 找到的原始文本 URL>' -o /tmp/<book>-full.txt
...
若完整句子 >250 字符 → 截断，但**截断后必须仍是语法完整的句子**。
...
4. **不得已时保留完整长句**：若上述规则无法产出 ≤250 字符的完整句子 → 保留完整原文（标注长度），sync_anki.py 会拒绝超长句子→返回重新处理
...
- **禁止截断切掉 `<b>` 标签**：截断必须在 `<b>…</b>` 之后进行，绝不能把生词本身裁掉。若生词在句子末尾 1/3 处且截断会导致 `<b>` 丢失 → 改用 2A-c-3 扩展策略重找更短的句子，或保留完整长句
...
4. 若上述任一为否 → 放弃截断，使用更短的替代句子（2A-c-3 扩展策略），或保留完整长句返回重处理

**翻译对应规则**：**`translation_cn` 必须只翻译截断后的最终 `sentence`**，不得翻译截断前的完整原文。若截断后句子缺少某部分内容，翻译也必须同步省略。

**源文本获取失败时：**
- curl 直链 + WebSearch/WebFetch 均无法获取 → **该批次所有单词跳过，不生成卡片**。Step 2H 汇总中标明 `源文本不可用，N 个单词未生成`
- **禁止回退到词典例句**。词典例句脱离书中语境，对阅读理解没有帮助。卡片的价值在于"这个词在这本书的这个句子里是这个意思"，没有源文本就没有卡片

**完成后进入 Step 2B**——候选句已提取，Claude 进行句子选择和完整性校验。

### Step 2F: DeepL 机械翻译

例句翻译全部交给 DeepL API 完成，Claude 不参与翻译也不校验翻译结果。

**前置条件**：`DEEPL_API_KEY` 环境变量（免费 key 从 https://www.deepl.com/pro-api 获取，500,000 字符/月）。**未设置则跳过翻译**，`translation_cn` 字段留空。
...
在 **2E 之后、2G 之前**执行。Claude 流程（2B→2C→2D→2E）全部完成后才调用 DeepL，中途中断不浪费翻译配额。

**完成后进入 Step 2B**——候选句已提取，开始 Claude 流程。

> 完整工作流参见 `<skill_dir>/lib/SHARED_WORKFLOW.md`。

### Step 2B: 句子选择 + 完整性校验（Claude，1 agent，先于 2D 执行）

从 match_sentences.py 输出的 candidates 中选句、校验完整性、极端长句截断。参见 SHARED_WORKFLOW.md Step 2B。

### Step 2C: IPA 预填充（cmudict 批量生成）

> `sync_anki.py` 验证要求 `ipa` 字段非空。在 Claude 生成释义之前用 cmudict 批量填充 IPA，3C 只处理 cmudict 未覆盖的词和异读词投票。
...
with open('/tmp/vocab-anki-input-<bookId>.json') as f:
    data = json.load(f)
...
cmudict 未覆盖的词由 Claude 在 3C 中手动补充。
...
- 例句应简洁：1-2 句，通常 ≤250 字符。**禁止**使用整段对话或长段落——仅提取目标词所在的核心句及其紧邻上下文，让学习者在 3 秒内定位到生词
- **以上规则由 `sync_anki.py` 在同步前自动校验**：句子长度 >250 字符、`<b>` 内容与 `word` 字段不匹配、必填字段（ipa/definition_cn/translation_cn）缺失均会拒绝同步并打印错误。尽早生成高质量内容，避免回滚重做
...
**同步后审计（Step 2H 完成后必做）**：逐张检查 Anki 卡片，重点查三类问题：
...
>
> **执行铁律**：读完 pipeline 输出后，**先执行 Step 2A 拉取源文本**，从源文本中机械匹配所有单词的句子。然后在写入 JSON 时直接填入已提取的句子。不要在 thinking block 中回忆例句——句子已从源文本检索到。
...
**自查清单**：已移至 Step 2E（独立验证 agent）。2D 生成 agent 只生成、不自查。
...
- **Agent 使用策略**：Step 2D 使用 N 个 Agent 并行生成（≤25 词/agent），Step 2B 和 2E 各使用 1 个 Agent 串行处理。Agent 之间通过临时 JSON 文件传递数据，无实时协调开销

### Step 2E: 内容验证（Claude，1 agent）

Step 2D 全部完成后，合并所有 batch 输出，由 1 个 agent 逐词验证：
...
- **句子来源**：所有例句均通过 Step 2A 从源文本机械提取。源文本不可用 → 跳过该批次所有单词，Step 2H 汇总中标明
...
输出末尾包含 `AUDIO_DIR=<path>` 行，提取路径供 Step 2H 使用。
...
- **音频状态**：Step 2G 结果（如 `word✓ 64/64, sent✓ 62/64`）
...
> 音频已在 Step 2G 预下载到临时目录。同步阶段仅上传媒体 + 创建卡片，无需等待音频生成。
...
1. Step 2G (`--prefetch`): 并发生成全部音频 → 保存到临时目录 + manifest.json → 打印 `AUDIO_DIR=<path>`
2. Step 2H (`--audio-dir <dir>`): 从目录加载预生成音频 → 连 AnkiConnect → 查已有卡片 → 上传媒体 → 添加新卡片
...
牌组名优先从 JSON `deck_name` 字段读取（Claude 在 Step 2D 从 Step 0b 的 `{牌组名: bookId}` 映射反查填入）。未提供时回退 `--deck` 参数；都未提供才自动拼接。
...
| 源文本不可用 | curl 直链 + WebSearch/WebFetch 均无法获取书中文本 → **该批次所有单词跳过**，Step 2H 汇总中标明 `源文本不可用，N 个单词未生成`。禁止回退到词典例句 |
...
- **职责分离**：Claude 做知识工作（理解语境、释义、IPA 异读词投票），DeepL 做机械翻译，Python 做机械工作（HTTP、TTS、同步）。句子提取由 Step 2A 源文本检索完成，不依赖 Claude 回忆
- **源文本检索替代回忆**：例句不再依赖 Claude 记忆生成，改为从网上拉取书中实际文本后机械匹配。Claude 的知识工作从「回忆句子+翻译+IPA」收窄为「释义+IPA 投票」，翻译全部交给第三方（DeepL），消除最易出错的环节
...
- **章节优先匹配**：filter_pipeline.py 透传 WeRead 的 `chapterUid`/`chapterTitle` 到 JSON 输出，Step 2A 优先在单词所属章节范围内搜索句子，避免同名异义词匹配到书中其他位置（如 `fair` 在"公平的"和"集市"两个义项间不会串章）。章节边界不确定时回退到全文本搜索
