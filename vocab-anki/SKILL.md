...
**例句规则（不变）：**
- 必须是书中真实句子，不是词典通用例句。句子来源只有一条路径：2A-c 源文本机械匹配
...
3. 164 句无需截断，75 句 >150 字符按 2A-e 规则截断，6 个词形变体手动定位
...
- **音频并发**：多线程（16 workers）并发生成音频（Step 2G），将音频生成压缩到秒级
