...
- **Two-layer lemmatization**: Step 1d `lemmatize_word()` handles inflectional only (for dedup — same word, different forms). Step 1f `in_coca()` in `lib/coca.py` fallback (lemminflect + suffix stripping) handles derivational normalization (for frequency lookup — `indulgently`→`indulgent`, `resentfulness`→`resentful`). The two layers serve different purposes and are complementary, not redundant. Without the COCA derivational layer, words like `indulgently` (COCA has `indulgent` but not the -ly form) would be incorrectly excluded. Both layers use `len(lemma) < len(word)` to avoid cross-POS false positives (e.g. `abode` n.≠`abide` v.). This unified guard correctly handles doubled-consonant patterns lemminflect misses (`crammed`→`cram`, `forsaken`→`forsake`) while rejecting same-length mismatches. Same-length irregular verbs (`ran`→`run`, `sat`→`sit`) are a known limitation but rarely highlighted as new vocab
