...
- **Two-layer lemmatization**: Step 1d `lemmatize_word()` handles inflectional only (for dedup — same word, different forms). Step 1f `in_coca()` fallback (lemminflect + suffix stripping) handles derivational normalization (for frequency lookup — `indulgently`→`indulgent`, `resentfulness`→`resentful`). The two layers serve different purposes and are complementary, not redundant. Without the COCA derivational layer, words like `indulgently` (COCA has `indulgent` but not the -ly form) would be incorrectly excluded. Lemminflect in `in_coca()` only accepted when `len(lemma) < len(word)` to avoid cross-POS false positives (e.g. `abode` n.≠`abide` v.). Same-length irregular verbs (`ran`→`run`, `sat`→`sit`) are a known limitation but rarely highlighted as new vocab
