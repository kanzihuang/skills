...
**在 2C/2D 生成释义之前执行**，避免为会被排除的词浪费后续时间。
...
> `sync_anki.py` 的 `_validate_word_entries()` 要求 `ipa` 字段非空（硬错误），但 cmudict 自动生成发生在 `_process_one_word()` 阶段（验证之后）。因此在 2D 生成释义之前用 cmudict 批量填 IPA，2D 只处理 cmudict 未覆盖的词和异读词投票。
...
| `definition_cn` | 格式 `[词性] 释义`。词性用 `[n.]` `[v.]` `[adj.]` `[adv.]` `[prep.]` `[conj.]`。按句中实际用法，禁止括号补充说明 |
