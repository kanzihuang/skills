...
> ⚠️ **不可绕过（MUST）**：此步骤无 SKIP 条件。即使 match_sentences.py 的机械预选看起来正确
> 也必须执行。Claude agent 的阅读能力是 Python 脚本无法替代的——只有 Claude 能识别：
> - 序言/非正文句子（char_offset 靠前、内容为作者简介/编辑导语）
> - 语法不完整的句子片段
> - 语义不匹配的候选句（如多候选句中选错）
>
> **禁止**在看到「无长句」「单选」时跳过此步骤。执行后运行检查点：
> ```bash
> python <skill_dir>/lib/scripts/check_step_completed.py /tmp/vocab-anki-input-<tmp_id>.json --step 2B
> ```

**在 2A 之后、2C(DeepL) 之前执行**，避免为会被排除的词浪费时间。
...
- `selected`（match_sentences.py 预选的最佳候选句，机械规则已选最优）
...
```
对每个 word:
  0. 使用 match_sentences.py 预选的 `selected` 句子
     → 若 selected 为 null → 跳过此词（excluded）
     预选规则（select_best_sentence，三档机械选择）：
       Tier 1: 30 ≤ len ≤ 250 → 选最短（理想区间，上下文充分无需截断）
       Tier 2: 仅剩 > 250 → 选最短（需语义截断，但优于无上下文的 < 30 短句）
       Tier 3: 全部 < 30 → 选最长（尽力而为，validation 软警告）

  1. 完整性检查：主谓结构、首字母大写、非名词短语
     → 失败 → 回退到 candidates 中其他候选句（按同机械规则尝试）
     → 全部失败 → 回源文本手动提取（步骤 4）
...
- **禁止改编**：截断只能从首尾删除，不得替换/增删句中词语。截断是取源文本句子的连续子串，不是改写。截断后必须通过 regex 源文本校验（自检第 0 步）

**截断后自检**（每句截断后立即执行）：
0. **regex 源文本校验**：用 `_build_sentence_regex(截断句)` 搜索源文本（通过 `char_offset` ±500 定位范围）。匹配 → 通过，截断是合法的连续子串。不匹配 → **驳回**：截断变成了改编，必须保留更多原文，退回重做
...
**`word` 必须是句子中 `<b>` 包裹的表面词形，不能是 match_sentences.py 输出的 `rep` 字段。**

`match_sentences.py` 输出中每个词有 `rep`（canonical form，即 filter 输出中的 lemma）和 `forms`（表面词形数组）。`rep` 用于搜索，但写入 JSON `word` 字段的必须是 `<b>` 标签中的实际文本。

```
❌ match_sentences.py 输出: rep="arouse", forms=["aroused"], <b>aroused</b>
   JSON 中 word="arouse"  ← 错误！sync_anki.py 校验 word ≠ <b> 文本
✅ JSON 中 word="aroused"   ← 正确，与 <b> 文本一致
```

**受影响词**：所有屈折形式与 `rep` 不同的词（`-ed`/`-ing`/`-s`/`-es` 等）。`sync_anki.py` 行 870 自动校验 `word` 与 `<b>` 文本一致性，不匹配的卡片会被拒绝。

**修复方式**：写入 JSON 前，对每个词执行 `re.search(r'<b>([^<]+)</b>', sentence).group(1)` → 写入 `word` 字段。
...
在 **2B 之后、2D 之前**执行，翻译先于 Claude 生成释义。翻译结果作为 2D 的句子语境参考。

前置条件：`DEEPL_API_KEY` 环境变量。**未设置则跳过翻译**，`translation_cn` 字段留空，2D 退回到无翻译模式。
...
用 2B 确定的最终句子和 2C(DeepL) 的翻译，为每个词生成 `definition_cn` + 手动补充 cmudict 未覆盖词的 IPA + 异读词投票。

**分批**：≤25 词/agent。≤30→1, 31–80→4, 81–200→8, 200+→10+。每批按字母序排列。
**不自查**——自查职责移至 2F。

**翻译辅助**：如果 `translation_cn` 字段已有值（Step 2C DeepL 翻译），参考翻译确认单词在句中的具体义项。释义应与翻译的语义保持一致。翻译未设置时正常生成释义。
...
| `ipa` | 大部分由 2E cmudict 填充（从最终 lemma 生成）；2D 异读词投票 + 2F 兜底 | 单发音词留空；异读词填 `/riːd/`（非 `/red/`）|
| `definition_cn` | **格式**：`[词性] 释义`。词性用 `[n.]` `[v.]` `[adj.]` `[adv.]` `[prep.]` `[conj.]`。释义**按句中实际用法**。**禁止**在释义后加括号补充说明 | `inquiry` 在 "came a timid inquiry"→`[n.] 询问，提问` |
| `translation_cn` | 整句中文翻译，由 Step 2C DeepL 提供。`DEEPL_API_KEY` 未设置时留空 | `我于是对丛林中的冒险深深思索起来。` |
...
## Step 2E: IPA 生成（cmudict，2D 之后）

> `sync_anki.py` 验证要求 `ipa` 字段非空。在 Claude 生成释义之前用 cmudict 批量填充 IPA，2E 只处理 cmudict 未覆盖的词和异读词投票。
...
for w in data['words']:
    if w.get('ipa') and '/' in w['ipa']:
        continue
    # Use final lemma from 2D (if set) or the word itself.
    # No more lemmatize() guessing — 2D already finalized the lemma.
    json_lemma = w.get('lemma', '').strip()
    lookup = json_lemma if json_lemma else w['word']
    cmu = _cmu_ipa(lookup)
    if cmu:
        w['ipa'] = cmu
    else:
...
if missing_cmudict:
    ...
    for m in missing_cmudict:
        print(f"  {m}")
    print("2F will fill these — cmudict miss, Claude to provide manually.")
...
cmudict 未覆盖的词由 Claude 在 2D 中手动补充，2F 兜底。

---

## Step 2F: 内容验证（Claude，1 agent）

> ⚠️ **不可绕过（MUST）**：此步骤无 SKIP 条件。自动检查（validation.py）只做格式校验，
> 不做语义校验。只有 Claude 能识别定义质量、翻译-释义对齐、共享句定义一致性。
> **禁止**用自动检查替代 Claude agent。执行前先跑检查点确认 2E 完成：
> ```bash
> python <skill_dir>/lib/scripts/check_step_completed.py /tmp/vocab-anki-input-<tmp_id>.json --step 2F
> ```
> （2F 入口检查用于确认 2D/2E 产出完整；2F 出口检查在 2F agent 完成后由主流程执行）

POS 对齐 + 释义准确 + 翻译一致性。发现问题 → 直接修正。
...
仅问一次，**必须使用以下模板**展示完整信息：

```
📋 牌组: {deck_name}
   新增卡片: {new_cards} 张
   排除: {excluded} 张（{排除原因}）
📦 媒体: {media_count} 个音频文件（单词 {word_audio} + 例句 {sent_audio}）
🎯 操作: 上传音频 → 添加 {new_cards} 张卡片 → 触发 AnkiWeb 同步
🔒 已有 {existing} 张卡片不受影响
...
3. 音频命名：`{lemma}_{<tmp_id>}_word.mp3` / `{lemma}_{<tmp_id>}_sent.mp3`。同批次内同词族的词共享同一文件名——`sync_anki.py` 在音频生成前去重，只保留第一个出现的词，第二个起跳过，避免文件名冲突
...
| 句子 | 长度 < 30 字符（MIN_SENTENCE_LENGTH）→ 上下文不足 | 软警告 |
| 例句 | 首字母小写 → 截断片段 | 软警告 |
| 例句 | 缺少定式动词 → 名词短语片段 | 软警告 |
| WordId | 批内重复 lemma（`seen_word_ids`）→ 第二个起静默跳过 | 静默跳过 |
...
- **批内词族去重**：同批次内同词族词（如 `boa`/`boas` 归并为 `boa`）在 `sync_anki.py` 音频生成前通过 `seen_word_ids` 去重，第一个出现者胜出。避免音频文件名冲突（两个词生成同一 `boa_{suffix}_sent.mp3`，后者覆盖前者）
