...
**在 2C 之后、2E 生成释义之前执行**，避免为会被排除的词浪费时间。
...
if missing_cmudict:
    ...
    for m in missing_cmudict:
        print(f"  {m}")
    print("2E must provide IPA manually for these words.")
...
cmudict 未覆盖的词由 Claude 在 2E 中手动补充。

---

## Step 2E: 生成内容（Claude，N agents 并行）

用 2B 确定的最终句子和 2D 的 DeepL 翻译，为每个词生成 `definition_cn` + 手动补充 cmudict 未覆盖词的 IPA + 异读词投票。

**分批**：≤25 词/agent。≤30→1, 31–80→4, 81–200→8, 200+→10+。每批按字母序排列。
**不自查**——自查职责移至 2F。

**翻译辅助**：如果 `translation_cn` 字段已有值（Step 2D DeepL 翻译），参考翻译确认单词在句中的具体义项。释义应与翻译的语义保持一致。翻译未设置时正常生成释义。

### 2E-0. lemma 判定（生成内容前必做）
...
**Claude 逐词判断**（Step 2E 生成每个词时，读它的 `_spacy` + `sentence`）：
...
| `ipa` | 大部分已由 2C cmudict 填充；2E 补充未覆盖词 + 异读词投票 | 单发音词留空；异读词填 `/riːd/`（非 `/red/`）|
| `definition_cn` | **格式**：`[词性] 释义`。词性用 `[n.]` `[v.]` `[adj.]` `[adv.]` `[prep.]` `[conj.]`。释义**按句中实际用法**。**禁止**在释义后加括号补充说明 | `inquiry` 在 "came a timid inquiry"→`[n.] 询问，提问` |
| `translation_cn` | 整句中文翻译，由 Step 2D DeepL 提供。`DEEPL_API_KEY` 未设置时留空 | `我于是对丛林中的冒险深深思索起来。` |
...
在 **2C 之后、2E 之前**执行，翻译先于 Claude 生成释义。翻译结果作为 2E 的句子语境参考。

前置条件：`DEEPL_API_KEY` 环境变量。**未设置则跳过翻译**，`translation_cn` 字段留空，2E 退回到无翻译模式。
